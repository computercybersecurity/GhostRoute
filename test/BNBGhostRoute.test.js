...
const GhostRoute = artifacts.require("./BNBGhostRoute.sol");
const { BNB_AMOUNT, MERKLE_TREE_HEIGHT } = process.env;

const websnarkUtils = require("websnark/src/utils");
const buildGroth16 = require("websnark/src/groth16");
const stringifyBigInts = require("websnark/tools/stringifybigint")
  .stringifyBigInts;
...
contract("BNBGhostRoute", (accounts) => {
  let ghostRoute;
  const sender = accounts[0];
  const operator = accounts[0];
  const levels = MERKLE_TREE_HEIGHT || 16;
  const value = BNB_AMOUNT || "1000000000000000000"; // 1 bnb
  let snapshotId;
  let prefix = "test";
  let tree;
  const fee = bigInt(BNB_AMOUNT).shr(1) || bigInt(1e17);
  ...
  before(async () => {
    ...
    ghostRoute = await GhostRoute.deployed();
  ...
  describe("#constructor", () => {
    it("should initialize", async () => {
      const bnbDenomination = await ghostRoute.denomination();
      bnbDenomination.should.be.eq.BN(toBN(value));
    });
  });
  ...
  describe("#deposit", () => {
    it("should emit event", async () => {
      ...
      let { logs } = await ghostRoute.deposit(commitment, {
        value,
        from: sender,
      });
      ...
      ({ logs } = await ghostRoute.deposit(commitment, {
        value,
        from: accounts[2],
      }));
    ...
    it("should throw if there is a such commitment", async () => {
      ...
      await ghostRoute.deposit(commitment, { value, from: sender }).should.be
        .fulfilled;
      const error = await ghostRoute.deposit(commitment, {
        value,
        from: sender,
      }).should.be.rejected;
  ...
  describe("#withdraw", () => {
    it("should work", async () => {
      ...
      // let gas = await ghostRoute.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await ghostRoute.deposit(toFixedHex(deposit.commitment), {
        value,
        from: user,
        gasPrice: "0",
      });
      ...
      balanceUserAfter.should.be.eq.BN(
        toBN(balanceUserBefore).sub(toBN(value))
      );
      ...
      const balanceGhostRouteBefore = await web3.eth.getBalance(
        ghostRoute.address
      );
      const balanceRelayerBefore = await web3.eth.getBalance(relayer);
      const balanceOperatorBefore = await web3.eth.getBalance(operator);
      const balanceRecieverBefore = await web3.eth.getBalance(
        toFixedHex(recipient, 20)
      );
      let isSpent = await ghostRoute.isSpent(toFixedHex(input.nullifierHash));
      isSpent.should.be.equal(false);

      // Uncomment to measure gas usage
      // gas = await ghostRoute.withdraw.estimateGas(proof, publicSignals, { from: relayer, gasPrice: '0' })
      ...
      const { logs } = await ghostRoute.withdraw(proof, ...args, {
        from: relayer,
        gasPrice: "0",
      });

      const balanceGhostRouteAfter = await web3.eth.getBalance(
        ghostRoute.address
      );
      const balanceRelayerAfter = await web3.eth.getBalance(relayer);
      const balanceOperatorAfter = await web3.eth.getBalance(operator);
      const balanceRecieverAfter = await web3.eth.getBalance(
        toFixedHex(recipient, 20)
      );
      const feeBN = toBN(fee.toString());
      balanceGhostRouteAfter.should.be.eq.BN(
        toBN(balanceGhostRouteBefore).sub(toBN(value))
      );
      balanceRelayerAfter.should.be.eq.BN(toBN(balanceRelayerBefore));
      balanceOperatorAfter.should.be.eq.BN(
        toBN(balanceOperatorBefore).add(feeBN)
      );
      balanceRecieverAfter.should.be.eq.BN(
        toBN(balanceRecieverBefore).add(toBN(value)).sub(feeBN)
      );
      ...
      isSpent = await ghostRoute.isSpent(toFixedHex(input.nullifierHash));
    ...
    it("should prevent double spend", async () => {
      ...
      await ghostRoute.withdraw(proof, ...args, { from: relayer }).should.be
        .fulfilled;
      const error = await ghostRoute.withdraw(proof, ...args, { from: relayer })
        .should.be.rejected;
    ...
    it("should reject with tampered public inputs", async () => {
      ...
      let error = await ghostRoute.withdraw(proof, ...incorrectArgs, {
        from: relayer,
      }).should.be.rejected;
      ...
      incorrectArgs = [
        ...
        toFixedHex(input.relayer, 20),
        toFixedHex(
          "0x000000000000000000000000000000000000000000000000015345785d8a0000"
        ),
        toFixedHex(input.refund),
      ];
      error = await ghostRoute.withdraw(proof, ...incorrectArgs, {
        from: relayer,
      }).should.be.rejected;
      error.reason.should.be.equal("Invalid withdraw proof");
      ...
      incorrectArgs = [
        toFixedHex(input.root),
        toFixedHex(
          "0x00abdfc78211f8807b9c6504a6e537e71b8788b2f529a95f1399ce124a8642ad"
        ),
        toFixedHex(input.recipient, 20),
        toFixedHex(input.relayer, 20),
        toFixedHex(input.fee),
      ...
      await ghostRoute.withdraw(originalProof, ...args, { from: relayer })
        .should.be.fulfilled;
    });
    ...
    it("should reject with non zero refund", async () => {
      ...
      error.reason.should.be.equal(
        "Refund value is supposed to be zero for BNB instance"
      );
    });
  });
  ...
  describe("#changeOperator", () => {
    it("should work", async () => {
      let operator = await ghostRoute.operator();
      operator.should.be.equal(sender);

      const newOperator = accounts[7];
      await ghostRoute.changeOperator(newOperator).should.be.fulfilled;

      operator = await ghostRoute.operator();
    ...
    it("cannot change from different address", async () => {
      ...
      const newOperator = accounts[7];
      const error = await ghostRoute.changeOperator(newOperator, {
        from: accounts[7],
      }).should.be.rejected;
  ...
  describe("#updateVerifier", () => {
    it("should work", async () => {
      ...
      await ghostRoute.updateVerifier(newVerifier).should.be.fulfilled;

      const verifier = await ghostRoute.verifier();
    ...
    it("cannot change from different address", async () => {
      ...
      const newVerifier = accounts[7];
      const error = await ghostRoute.updateVerifier(newVerifier, {
        from: accounts[7],
      }).should.be.rejected;
      error.reason.should.be.equal("Only operator can call this function.");
    });
  ...
  describe("#isSpent", () => {
    it("should work", async () => {
      ...
      await ghostRoute.deposit(toFixedHex(deposit1.commitment), {
        value,
        gasPrice: "0",
      });
      await ghostRoute.deposit(toFixedHex(deposit2.commitment), {
        value,
        gasPrice: "0",
      });
      ...
      await ghostRoute.withdraw(proof, ...args, {
        from: relayer,
        gasPrice: "0",
      });

      const nullifierHash1 = toFixedHex(
        pedersenHash(deposit1.nullifier.leInt2Buff(31))
      );
      const nullifierHash2 = toFixedHex(
        pedersenHash(deposit2.nullifier.leInt2Buff(31))
      );
      const spentArray = await ghostRoute.isSpentArray([
        nullifierHash1,
        nullifierHash2,
      ]);

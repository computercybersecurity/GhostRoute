...
const GhostRoute = artifacts.require("./BEP20GhostRoute.sol");
const BadRecipient = artifacts.require("./BadRecipient.sol");
const Token = artifacts.require("./BEP20Mock.sol");
...
contract("BEP20GhostRoute", (accounts) => {
  let ghostRoute;
  let token;
  let usdtToken;
  let badRecipient;
  ...
  let tokenDenomination = TOKEN_AMOUNT || "1000000000000000000"; // 1 bnb
  let snapshotId;
  let prefix = "test";
  let tree;
  const fee = bigInt(BNB_AMOUNT).shr(1) || bigInt(1e17);
  const refund = BNB_AMOUNT || "1000000000000000000"; // 1 bnb
  ...
  before(async () => {
    ...
    if (BEP20_TOKEN) {
      token = await Token.at(BEP20_TOKEN);
      usdtToken = await USDTToken.at(BEP20_TOKEN);
    } else {
  ...
  describe("#constructor", () => {
    it("should initialize", async () => {
      const tokenFromContract = await ghostRoute.token();
  ...
  describe("#deposit", () => {
    it("should work", async () => {
      ...
      await token.approve(ghostRoute.address, tokenDenomination);

      let { logs } = await ghostRoute.deposit(commitment, { from: sender });
    ...
    it("should not allow to send bnb on deposit", async () => {
      const commitment = toFixedHex(43);
      await token.approve(ghostRoute.address, tokenDenomination);

      let error = await ghostRoute.deposit(commitment, {
        from: sender,
        value: 1e6,
      }).should.be.rejected;
      error.reason.should.be.equal(
        "BNB value is supposed to be 0 for BEP20 instance"
      );
    });
  });
  ...
  describe("#withdraw", () => {
    it("should work", async () => {
      ...
      // let gas = await ghostRoute.deposit.estimateGas(toBN(deposit.commitment.toString()), { from: user, gasPrice: '0' })
      ...
      const balanceGhostRouteBefore = await token.balanceOf(ghostRoute.address);
      ...
      const balanceGhostRouteAfter = await token.balanceOf(ghostRoute.address);
      ...
      balanceGhostRouteAfter.should.be.eq.BN(
        toBN(balanceGhostRouteBefore).sub(toBN(tokenDenomination))
      );
    ...
    it("should reject with wrong refund value", async () => {
      ...
      let { reason } = await ghostRoute.withdraw(proof, ...args, {
        value: 1,
        from: relayer,
        gasPrice: "0",
      }).should.be.rejected;
      ...
      ({ reason } = await ghostRoute.withdraw(proof, ...args, {
    ...
    it.skip("should work with REAL USDT", async () => {
      ...
      await usdtToken.approve(ghostRoute.address, tokenDenomination, {
        from: user,
      });
      console.log("approve done");
      const allowanceUser = await usdtToken.allowance(user, ghostRoute.address);
      ...
      const balanceGhostRouteBefore = await usdtToken.balanceOf(
        ghostRoute.address
      );
      ...
      let isSpent = await ghostRoute.isSpent(
      ...
      const balanceGhostRouteAfter = await usdtToken.balanceOf(
        ghostRoute.address
      );
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN);
      isSpent = await ghostRoute.isSpent(
        input.nullifierHash.toString(16).padStart(66, "0x00000")
      );
      isSpent.should.be.equal(true);
    });

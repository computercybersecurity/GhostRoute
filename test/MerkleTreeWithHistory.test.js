...
const MerkleTreeWithHistory = artifacts.require(
...
contract("MerkleTreeWithHistory", (accounts) => {
  ...
  // eslint-disable-next-line no-unused-vars
  const value = BNB_AMOUNT || "1000000000000000000";
  let snapshotId;
  let prefix = "test";
  let tree;
  let hasher;
    ...
    it("should work", async () => {
      ...
      for (let i = 1; i < 5; i++) {
        ...
        let isKnown = await merkleTreeWithHistory.isKnownRoot(
          toFixedHex(path.root)
        );
        isKnown.should.be.equal(true);
      }

      await merkleTreeWithHistory.insert(toFixedHex(42), { from: sender })
        .should.be.fulfilled;
      // check outdated root
      let isKnown = await merkleTreeWithHistory.isKnownRoot(
        toFixedHex(path.root)
